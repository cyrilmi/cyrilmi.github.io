...
This script performs a data set simulation and OPSCR modelling in the presence of sampling interuptions with NIMBLE [@NIMBLE; @deValpine2017]. All details about the procedure are provided in Milleret et al. Estimating abundance with interruption in data collection: Simulations and case studies using open population spatial capture-recapture models.
...
Download the .R data file Containing the R functions [here](http://cyrilmi.github.io/Interruptions/SourceRFunctions.R), the Nimble functions [here](http://cyrilmi.github.io/Interruptions/SourceNimbleObservationModel.R) and [here](http://cyrilmi.github.io/Interruptions/SourceNimbleObservationModel.R) 
...
```{r source, echo=F,message=FALSE,warning=FALSE,cache.comments=TRUE}
...
### ==== 3.SIMULATE INDIVIDUAL STATE MATRIX : z.mx ====
### ====  3.1 DEFINE PHI AND RHO ARRAYS ====  
# CREATE ARRAYS SO PHI AND FEC CAN VARRY OVER TIME AND STOCHASTICITY IN VITAL RATES CAN BE ADDED (t)
...
# WITH A SD ("normSD") EQUAL TO THE DISPERSAL SIGMA ("dispSigma")  
...
# id=5
# for(t in 1:5){
#    plot(myScaled$grid.scaled.xy, xlim=c(0,2), ylim=c(0,2))
# 
# sxy[id,,t]
# points(sxy[id,2,t] ~ sxy[id,1,t],col="red")
# points(myScaled$data.scaled.xy[which(y[id,,t]>0),],  pch=16 )
# }
...
myNimbleOutput <- runMCMC(mcmc = cMCMC,
                          nburnin = 1000, niter = 5000, nchains = 2, samplesAsCodaMCMC = TRUE)
...
coords <- matrix(c(0                   , 0                   ,
                   (grid.size + buffer*2)/ ((grid.size + buffer*2)/4), 0                   ,
                   (grid.size + buffer*2)/ ((grid.size + buffer*2)/4), (grid.size + buffer*2)/ ((grid.size + buffer*2)/4),
                   0                   , (grid.size + buffer*2)/ ((grid.size + buffer*2)/4),
                   0                   , 0 
), ncol = 2, byrow = TRUE)
...
myNimbleOutputdf <- do.call(rbind,myNimbleOutput)
plot(myStudyArea)
...
par(mfrow=c(2,3),mar=c(0,0,3,0))
for(t in 1:5){
  plot(myStudyArea,main=t)
  
for(i in 7000:dim(myResults$sims.list$sxy)[1]){
yy<- myResults$sims.list$sxy[i,,2,t] * (myResults$sims.list$z[i,,t]==2)
xx<- myResults$sims.list$sxy[i,,1,t] * (myResults$sims.list$z[i,,t]==2)
points(xx~yy, pch=16, cex=0.6, col=adjustcolor("red",alpha.f = 0.1) ,bg=adjustcolor("red",alpha.f = 0.1) )
}
points(nimData$detector.xy[,2]~nimData$detector.xy[,1], pch=16, ,bg=adjustcolor("black",alpha.f = 0.1) )
}
